...
class quickstack::ceph::config (
  ...
  #$rgw_host_names          = [ ],
  $rgw_keyring_filename    = '/etc/ceph/ceph.client.radosgw.keyring',
  $rgw_config_suffix       = 'radosgw.gateway',
  $rgw_hostname            = '',
  $rgw_dns_name            = '',
  $rgw_swift_url           = '',
  ...
  if $rgw_hostname {
    $_rgw_hostname = $rgw_hostname
  } else {
    $_rgw_hostname = $::hostname
  }
  ...
  if $rgw_key {
    File['etc-ceph'] ->
    ::quickstack::ceph::keyring_config{ "${rgw_config_suffix}":
      key              => $rgw_key,
      caps_mon         => 'allow rwx',
      caps_osd         => 'allow rwx',
